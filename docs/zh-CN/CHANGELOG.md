...
- **依赖**：在 `deno.json` 与 `package.json` 中将 React / react-dom 的 caret
  范围统一为 `^19.2.6`，避免 Deno 解析 CLI 或 SSR 依赖图时触发 React 19
  运行时版本不一致错误。
...
- **依赖**：将 @dreamer/view 更新至 ^1.1.3，以兼容 view 动态子节点单节点优化
  （无包装 span）。
